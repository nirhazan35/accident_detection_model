...
if __name__ == "__main__":
    ...
    # Get feature sizes from first sample
    first_sample = features[0][0]  # First sequence, first frame
    backbone_features_size = len(first_sample["backbone_features"])
    neck_features_size = len(first_sample["neck_features"])
    
    logging.info(f"Detected feature sizes - Backbone: {backbone_features_size}, Neck: {neck_features_size}")
    ...
    # Initialize model with detected feature sizes
    model = LSTM(
        backbone_features_size=backbone_features_size,
        neck_features_size=neck_features_size,
    ...
    for epoch in range(EPOCHS):
        ...
        logging.info(f"AP Score: {ap_score:.4f}, Best Recall: {best_recall:.4f} at threshold {best_threshold:.4f}")
        ...
        if best_recall > best_val_recall:
            ...
            logging.info(f"Saved new best model with recall {best_recall:.4f}")
            
            # Analyze and visualize features for the best model
            feature_analysis = model.analyze_features()
            model.visualize_features(f"models/feature_importance_{timestamp}.png")
            logging.info("Feature importance analysis saved to models/feature_importance_{timestamp}.png")
    ...
    logging.info("Training complete. Metrics saved to training_metrics.png")